...
root:
	...
	if [ -f "$(RELEASE_ROOT)/modules/$(KERNEL_VERSION)/.mods_depend" ]; then \
		cat $(RELEASE_ROOT)/modules/$(KERNEL_VERSION)/.mods_depend | \
			sed '2,20s#\(.*\)#insmod /lib/modules/$(KERNEL_VERSION)/\1.ko#' >> $(INIT_FILE); \
	...
	#----------------dynamic so end.-------------------#
	sed -i 's/mi_common/insmod \/lib\/modules\/$(KERNEL_VERSION)\/mi_common.ko/g' $(INIT_FILE); \
	echo "#mi module" >> $(INIT_FILE);
	...
	echo /customer/run_later.sh >> $(OUTPUTDIR)/rootfs/etc/profile;	\
